...
| Regex tipe sel — `[a-z]+` vs `[a-zA-Z]+` | **Bug nyata, sudah diperbaiki di kode Bagian 2 di bawah.** Draf awal spec menulis `/\bt="([a-z]+)"/` (huruf kecil saja). Ditemukan dengan membangun berkas ZIP+XML uji sungguhan (bukan cuma dibaca) dan menjalankan `_unzipSheet`→`_parseSheetXml` end-to-end lewat Node: nilai atribut standar XLSX untuk sel teks-langsung adalah `inlineStr` (S besar) — regex lama gagal mencocokkannya, `type` jadi `null`, sel jatuh ke cabang "angka" dan menghasilkan `null`/`NaN` untuk teks apa pun. **Ini bukan kasus tepi jarang — `inlineStr` adalah bentuk UTAMA yang disebutkan `docs/HANDOFF-DESKTOP.md` untuk berkas GLP039 sungguhan**, jadi bug ini akan merusak SEMUA teks (nama program/kegiatan/item) di file SAKTI asli, bukan cuma kasus uji buatan. Regex final: `/\bt="([a-zA-Z]+)"/`. |
